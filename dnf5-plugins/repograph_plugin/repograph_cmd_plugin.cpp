...
constexpr const char * PLUGIN_NAME{"repograph"};
...
constexpr const char * attrs_value[]{
    "DNF5 contributors",
    "packaging-team@lists.fedoraproject.org",
    "repograph command: emit the package dependency graph in dot or JSON format."};

class RepographCmdPlugin : public IPlugin {
public:
...
std::vector<std::unique_ptr<Command>> RepographCmdPlugin::create_commands() {
    std::vector<std::unique_ptr<Command>> commands;
    commands.push_back(std::make_unique<RepographCommand>(get_context()));
    return commands;
}
...
IPlugin * dnf5_plugin_new_instance([[maybe_unused]] ApplicationVersion application_version, Context & context) try {
    return new RepographCmdPlugin(context);
} catch (...) {
